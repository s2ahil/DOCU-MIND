...
const FileUpload = () => {
  ...
  const onFileUpload = async () => {
    ...
    try {
      const response = await axios.post(
        "https://docu-mind-updated.vercel.app/process_pdf",
        formData,
        {
          headers: {
      ...
      setMessage(
        `PDF processing failed: ${error.response?.data?.detail || error.message}`,
      );
      setIsPdfProcessed(false);
    } finally {
  ...
  const onAskQuestion = async () => {
    ...
    try {
      const response = await axios.post(
        "https://docu-mind-updated.vercel.app/answer_question",
      ...
      setMessage(
        `Failed to get answer: ${error.response?.data?.detail || error.message}`,
      );
    } finally {
      setIsProcessing(false);
    }
  ...
  return (
    <div className="w-full h-full p-6 bg-white shadow-lg rounded-lg ">
      ...
      <div className="mb-8">
        ...
        {files.length > 0 && isPdfProcessed && (
          <div className="mb-3 flex items-center gap-2 text-sm text-gray-700">
            <span className="font-semibold">Current PDF:</span>
        ...
        <div className="flex space-x-4">
          <input
            ...
            className={`flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
              !isPdfProcessed ? "bg-gray-100" : ""
            }`}
          />

          <button
            className={`px-4 py-2 rounded-lg text-white font-semibold ${
              !isPdfProcessed || isProcessing || !question
